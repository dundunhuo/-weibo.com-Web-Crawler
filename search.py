...
def query_weibo_first_page(search_string, header, st, et, rest_time=(2, 5)):
    url = f"https://s.weibo.com/weibo?q={quote(search_string)}&typeall=1&suball=1&timescope=custom:" \
          f"{st[0]}-{st[1]}-{st[2]}-{st[3]}:{et[0]}-{et[1]}-{et[2]}-{et[3]}&Refer=g"
...
def query_weibo(search_string, header, st, et, page, rest_time=(2, 5)):
    url = f"https://s.weibo.com/weibo?q={quote(search_string)}&typeall=1&suball=1&timescope=custom:" \
...
def get_posts(db, table, web_text):
    ...
    for post in posts:
        ...
        try:
            ...
            pass
        try:
            # TODO: get full text if found "展开c" in fetched content
...
def search(db, table, query, start_time, end_time):
    connection = sqlite3.connect(db)
    ...
    response, max_page = query_weibo_first_page(
        search_string=query, header=chrome_113, st=start_time, et=end_time)
    get_posts(db=db, table=table, web_text=response)
    pbar = tqdm(total=max_page)
    pbar.update(1)
    for page in range(2, max_page + 1):
        response = query_weibo(search_string=query, header=chrome_113, st=start_time, et=end_time, page=page)
        if response:
            get_posts(db=db, table=table, web_text=response)
        pbar.update(1)
    pbar.close()
...
if __name__ == '__main__':
    search(
        ...
        table="star_rail",
        query="星穹铁道",
        start_time=(2023, 4, 26, 0),
        end_time=(2023, 4, 26, 3)
    )
